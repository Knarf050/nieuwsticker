...
const EXCLUDE_URL = /\/(sport|voetbal|wielrennen|schaatsen|tennis|formule-?1|darts|golf|hockey|wielersport|cultuur|media|muziek|film|films|boeken|kunst|entertainment|achterklap|lifestyle|koken|recept|reizen|wonen|relatie|dieren|royalty|koningshuis|opmerkelijk|puzzel|spelletjes|denksport)\b/i;
const EXCLUDE_TITLE = /\b(doelpunt|topscorer|eredivisie|champions league|europa league|conference league|bondscoach|rode kaart|gele kaart|transfervrij|landstitel|wereldtitel|pole position|grand prix)\b/i;
...
const RUBRIEK_TITLES = new Set([
  ...
  'denksport', 'spelregels', 'het kruiswoord',
]);
...
export function isExcluded(a) {
  ...
  if (EXCLUDE_TITLE.test(a.rawTitle || a.title || '')) return true;
