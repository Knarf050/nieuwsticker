...
// Accentkleur voor handgekozen aanbevelingen (geopolitiek & tech/AI).
export const REC_COLOR = '#7A5CFF';

// Zet de handgekozen aanbevelingen om naar artikel-objecten. Deze omzeilen de
// nieuws-filters én het 24-uursvenster; ze verschijnen met een 'Aanbevolen'-label.
// `sortDate` (= wanneer aanbevolen) bepaalt de volgorde, `pubDate` blijft de
// echte publicatiedatum voor de weergegeven tijd.
export function recommendedArticles(max = 5) {
  if (!Array.isArray(RECOMMENDATIONS)) return [];
  return RECOMMENDATIONS.slice(0, max).map(r => ({
    title: r.title,
    rawTitle: r.title,
    link: (r.link || '#').trim(),
    pubDate: r.pubDate || r.added || new Date().toISOString(),
    sortDate: r.added || r.pubDate || new Date().toISOString(),
    summary: r.summary || '',
    why: r.why || '',
    categories: [],
    source: r.source || 'Aanbevolen',
    color: REC_COLOR,
    recommended: true,
  }));
}

// Voeg de aanbevelingen toe aan een nieuwslijst en sorteer op tijd. Aanbevelingen
// gebruiken hun `sortDate` (aanbevelingsmoment) zodat verse tips bovenaan meelopen;
// dubbele links t.o.v. het gewone nieuws worden overgeslagen.
export function withRecommendations(articles, max = 5) {
  const recs = recommendedArticles(max);
  if (recs.length === 0) return articles;
  const seen = new Set(articles.map(a => (a.link || '').trim()));
  const merged = articles.concat(recs.filter(r => !seen.has(r.link)));
  merged.sort((a, b) =>
    new Date(b.sortDate || b.pubDate) - new Date(a.sortDate || a.pubDate));
  return merged;
}
...
export const SOURCES = [
  ...
  // NRC verwijderd: de algemene feed bevatte te veel niet-nieuws (opinie,
  // columns, puzzels, weekrubrieken) en is met regels niet schoon te krijgen.
];
...
const RUBRIEK_TITLES = new Set([
  ...
  'denksport', 'spelregels', 'het kruiswoord', 'aan zet', 'keob',
]);
...
// Terugkerende rubrieken met wisselende kop, herkenbaar aan de samenvatting-intro.
const EXCLUDE_SUMMARY = /\belke week luncht nrc\b/i;
...
export function isExcluded(a) {
  ...
  if (a.summary && EXCLUDE_SUMMARY.test(a.summary)) return true; // terugkerende rubriek
  return false;
}
...
// Haal een uniek artikelnummer uit de URL. Zo herkennen we hetzelfde artikel
// ook als de kop (en dus de slug) herschreven is — bijv. NU.nl dat één artikel
// onder twee verschillende koppen plaatst (zelfde nummer, andere slug).
function articleId(link) {
  if (!link) return '';
  let m = link.match(/nu\.nl\/[^/]+\/(\d{5,})(?:[/.]|$)/i);
  if (m) return 'nu:' + m[1];
  m = link.match(/nos\.nl\/(?:artikel\/|l\/|liveblog\/)?(\d{6,})/i);
  if (m) return 'nos:' + m[1];
  m = link.match(/nrc\.nl\/.*-a(\d{5,})/i);
  if (m) return 'nrc:' + m[1];
  return '';
}
...
// Ontdubbelt op artikelnummer (zelfde artikel, andere kop), exacte titel/link,
// en bijna-gelijke titels ("start zaak" vs "start rechtszaak").
export function dedupe(list) {
  ...
  for (const a of list) {
    ...
    const idKey = articleId(a.link);
    if (idKey && seen.has(idKey)) continue; // zelfde artikel, andere kop/slug
    if (!key) { if (idKey) seen.add(idKey); out.push(a); tokenSets.push(new Set()); continue; }
